...
/**
 * Per-session facade over [DesktopPersistentShell]. Same surface as the inner
 ...
 *
 * Structurally identical to androidMain's SessionShell, wrapping
 * DesktopPersistentShell instead of PersistentSandboxShell. Not moved to
 * jvmShared: keeping desktopMain/androidMain fully independent here avoids
 * touching the Android implementation, which is out of scope for this plan.
 */
class DesktopSessionShell(
    val sessionId: String,
    private val inner: DesktopPersistentShell,
     ...
     *   callers that wrap a user command should pass the original unwrapped
     *   form so the user sees what they actually asked for, not the plumbing.
     */
    suspend fun run(
        command: String,
        timeoutSeconds: Long,
     ...
     * bounded-trim. Pausing during a drag-select prevents SelectionManager
     * from crashing on a selectable id whose Text was pruned mid-drag.
     * Unpause runs a catch-up trim so memory stays bounded once the gesture ends.
     */
