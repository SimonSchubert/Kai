...
- **The file browser is shared with Kai Build**: the same browser — listing, breadcrumbs, editor, rename/delete, open-with — is pointed at whichever Linux environment the surface belongs to. Here it shows the Alpine sandbox from the filesystem root; in Kai Build it shows one project folder. The two never share a position or an open file.
