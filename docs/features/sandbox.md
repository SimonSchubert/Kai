...
- **Cancel escalates to children, same as Android.** Cancelling sends `SIGINT`, then `SIGTERM`, then `SIGKILL` to every descendant of the persistent shell (not the shell itself), stopping as soon as the foreground command exits. Only if `SIGKILL` doesn't shake it loose does the shell get reset. Desktop finds the pids to signal by walking the real JVM child process's descendants directly, rather than Android's pid-probe-plus-sibling-proot approach — there's no chroot layer in the way here, so a plain process handle is enough.
...
- **The assistant's shell tool shares the same session machinery as the Terminal tab and the Packages UI.** Desktop already had its own `execute_shell_command`/`manage_process` tools (predating the Dev Tools sandbox) that ran directly against the host with no session concept, gated by a generic settings toggle unrelated to sandbox state at all. They're now wired through the same per-session persistent shell as everything else on this page, gated on the sandbox actually being installed — the same fix Android already had via its own tool implementation. If the assistant reports commands succeeding but `which <tool>` resolves to a host path outside `~/.kai/linux-sandbox/micromamba/root/bin/`, that's this wiring being broken again — check `Platform.jvm.kt`'s tool-registration gate first.

## Behavior

The following bullets describe Android's Alpine/`proot` mechanics specifically; desktop's equivalent flow is in [Desktop Linux ("Dev Tools")](#desktop-linux-dev-tools) above. Both platforms now gate the assistant's sandbox tools on install state the same way (not sent to the model until the sandbox reports Ready) — see the note above about desktop's tool wiring for why this wasn't always true.
...
| `composeApp/src/desktopMain/kotlin/com/inspiredandroid/kai/tools/ShellCommandTool.kt` | The `execute_shell_command` tool the assistant calls on desktop. Routes through `DesktopSandboxController.shellFor()` — same per-session persistent shell as the Terminal UI. |
| `composeApp/src/desktopMain/kotlin/com/inspiredandroid/kai/tools/ProcessManager.kt` / `ProcessManagerTool.kt` | Background-job lifecycle on desktop: each background job gets its own dedicated sandbox session (not the conversation's), takes `shellFor`/`closeSession` as function parameters rather than a concrete controller so it's testable without Koin. |
| `composeApp/src/desktopMain/kotlin/com/inspiredandroid/kai/sandbox/DesktopPersistentShell.kt` | Desktop's persistent shell: an unconfined `bash` process with the installed environment's `bin/` on `PATH`. Same sentinel-based command-framing protocol as Android's `PersistentSandboxShell`, no chroot/proot layer. Graduated `SIGINT`/`SIGTERM`/`SIGKILL` cancel via `ProcessHandle.descendants()` instead of a pgrep/sibling-proot probe. |
